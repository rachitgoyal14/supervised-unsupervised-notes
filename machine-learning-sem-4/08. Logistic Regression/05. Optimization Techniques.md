...
> **Good news**: Although there is no closed-form solution, the log loss cost function is **convex**. This means gradient descent (or any other optimization algorithm) is **guaranteed to find the global minimum** (if the learning rate is not too large and you wait long enough).
...
This update rule is remarkably similar to linear regression's gradient descent — the only difference is that the prediction $p^{(i)}$ comes from the sigmoid function rather than a direct linear computation. The gradient computes the **prediction error** $(p^{(i)} - y^{(i)})$, multiplies it by the $j$-th feature value $x_j^{(i)}$, and averages over all training instances.
...
For **stochastic GD**, you take one instance at a time. For **mini-batch GD**, you use a mini-batch at a time. For **batch GD**, you use all training instances for each update step.
...
Regularization prevents overfitting by penalizing large weight values. The `C` parameter in scikit-learn controls regularization strength — it is the **inverse** of regularization strength: smaller values specify stronger regularization.

- **L1 (Lasso)**: Produces sparse models by forcing some weights to exactly zero. Use `penalty='l1'` with `solver='liblinear'` or `'saga'`.
- **L2 (Ridge)**: Shrinks all weights toward zero but rarely to exactly zero. Default in scikit-learn (`penalty='l2'`).
- **ElasticNet**: Combines L1 and L2. Use `penalty='elasticnet'` with `solver='saga'` and set `l1_ratio`.
