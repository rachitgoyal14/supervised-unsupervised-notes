...
A sigmoid function is any mathematical function whose graph has a characteristic **S-shaped** or sigmoid curve. The logistic/sigmoid function is used because of its behavior of **compressing the output between 0 and 1**.

> **Tip**: In the context of artificial neural networks, the term "sigmoid function" is used as a synonym for "logistic function".
...
**Case 1: When $z \to -\infty$**

$$\sigma(z) = \frac{1}{1 + e^{-z}} \to \frac{1}{1 + \infty} \to 0$$

As $z$ becomes very negative, the sigmoid output approaches 0. The model is very confident the instance belongs to class 0.

**Case 2: When $z = 0$**

$$\sigma(0) = \frac{1}{1 + e^{0}} = \frac{1}{1 + 1} = 0.5$$

At $z = 0$, the output is exactly 0.5 — the model is equally uncertain between both classes. This is the **decision boundary**.

**Case 3: When $z \to +\infty$**

$$\sigma(z) = \frac{1}{1 + e^{-z}} \to \frac{1}{1 + 0} \to 1$$

As $z$ becomes very positive, the sigmoid output approaches 1. The model is very confident the instance belongs to class 1.
...
6. **Bounded**: It is a bounded, differentiable, real function that is defined for all real input values and has a positive derivative at each point

Key observations from the graph:
- Sigmoid functions most often show a return value (y-axis) in the range 0 to 1
- The output can be directly interpreted as a probability of class membership
...
So probability = **88%** → predict **class 1**
